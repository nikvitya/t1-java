...
import ru.t1.java.firstService.dto.metric.NewMetricDto;
import ru.t1.java.firstService.dto.transaction.NewDataSourceErrorLogDto;
import ru.t1.java.general.dto.ClientDto;
import ru.t1.java.general.enums.transaction.NewTransactionDto;
import ru.t1.java.general.enums.transaction.TransactionAcceptDto;
import ru.t1.java.general.kafka.KafkaClientProducer;
import ru.t1.java.general.kafka.KafkaDataSourceExceptionProducer;
import ru.t1.java.general.kafka.KafkaMetricProducer;
...
public class KafkaConfg<T> {
    ...
    public KafkaClientProducer producerClient(@Qualifier("client") KafkaTemplate<String, ClientDto> template) {
        template.setDefaultTopic(config.clientRegisteredTopic);
    ...
    public KafkaMetricProducer metricProducer(KafkaTemplate<String, NewMetricDto> template) {
        template.setDefaultTopic(config.metricTopic);
    ...
    private Map<String, Object> generateDefaultProps(String valueDefaultType) {
        ...
        props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, config.maxPollIntervalsMs);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, Boolean.FALSE);
        props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
        props.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG, config.heartbeatInterval);
